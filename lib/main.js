...
async function run() {
  try {
    ...
    const pkg_file_name = core.getInput('pkg_file_name', { required: false })

    const pkgfile = path.join(process.env.GITHUB_WORKSPACE, pkg_root, pkg_file_name)
    ...
    const versionKey = core.getInput('version_key', { required: false })

    core.setOutput('version', pkg[versionKey])
    core.debug(` Detected version ${pkg[versionKey]}`)
    ...
    for (let tag of tags.data) {
      if (tag.name === getTagName(pkg[versionKey])) {
    ...
    const tagName = getTagName(pkg[versionKey])
    ...
    let newTag
    try {
      tagMsg = tagMsg.trim().length > 0 ? tagMsg : `Version ${pkg[versionKey]}`
